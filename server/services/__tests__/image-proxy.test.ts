...
describe('configured image proxy boundary', () => {
  ...
  it('never accepts a remote proxy, credentials, URL paths or unsupported protocol', () => {
    expect(parseImageProxy('127.0.0.1:7890')).toBe('http://127.0.0.1:7890')
    expect(parseImageProxy('https://[::1]:7890')).toBe('https://[::1]:7890')
    for (const value of ['http://192.168.1.1:7890', 'http://proxy.test:8080', 'socks5://127.0.0.1:1080', 'http://user:secret@127.0.0.1:7890', 'http://127.0.0.1:7890/path', 'http://127.0.0.1:7890?x=1']) expect(() => parseImageProxy(value)).toThrow()
  })
