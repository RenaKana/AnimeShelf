...
源码裁剪不删除已有数据库中的历史设置、凭据、任务或旧表；普通设置读取继续脱敏秘密字段，此桌面候选不包含手机同步或配置导出。外部 API 继续执行 `read` / `edit` / `files` 权限和所有者管理边界。
...
恢复到一个全新数据库时，只导入该版本已存在的表。若备份含有源码已裁剪模块的新表，应先用包含该模块的版本恢复，再关闭或裁剪模块；已有数据库中的保留表不受源码裁剪影响。
...
本候选不附私有工程的历史验收报告；上述命令是开发验证入口，不代表本候选已运行完整矩阵。按实际改动选择必要的组合。
